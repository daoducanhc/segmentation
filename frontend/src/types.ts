...
export interface CriteriaItem {
  ...
  label: string;
  field: string;
  category: 'activity' | 'payment' | 'churned' | 'profile' | 'revenue' | 'vip';
  defaultOperator: number;
  valueType: 'bool' | 'string' | 'number' | 'list';
}

export const CRITERIA_CATALOG: CriteriaItem[] = [
  // Activity (uses user_activity_summary - predefined windows)
  ...
  // Payment - PU (uses user_activity_summary - predefined windows)
  ...
  // Payment - NPU (Non-Paying User - NEVER paid, uses users table)
  { id: 'npu', label: 'NPU (Never Paid)', field: 'is_paying_user', category: 'payment', defaultOperator: 1, valueType: 'bool' },
  // Churned (uses user_activity_summary - predefined windows)
  ...
  // VIP (uses user_daily_activity - max_vip_level aggregated by app)
  { id: 'app_id', label: 'App ID', field: 'app_id', category: 'vip', defaultOperator: 7, valueType: 'list' },
  { id: 'vip_level', label: 'Max VIP Level', field: 'max_vip_level', category: 'vip', defaultOperator: 4, valueType: 'number' },
];
